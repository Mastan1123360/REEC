...
Everything under `lib/content/`, `lib/semantic/`, `lib/widgets/`,
`app/lesson/`, `app/phase/`, `app/upload/`, and the dashboard is a
separate, markdown-driven curriculum system predating this rebuild —
lesson authors write `.md` files with front matter and `:::block`
directives, and the engine compiles them into interactive study
sessions. This system is unrelated to Rust execution architecture and
was left untouched; its own `git log` / prior documentation covers it
in depth. The only integration point between the two systems is that
lesson widgets (`WorkedExample`, `SmartCode`, `MiniChallenge`) open the
Rust workspace panel when a learner clicks Run.
