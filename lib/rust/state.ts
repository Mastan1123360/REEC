...
interface RustWorkspaceState {
  ...
  openPanel: (source: string, title: string) => void;
  closePanel: () => void;
...
export const useRustWorkspace = create<RustWorkspaceState>()(
  persist(
    (set, get) => ({
      ...
      openPanel: (source, title) =>
        set((s) => {
          const target =
            s.project.files.find((f) => f.kind === "main") ??
            s.project.files.find((f) => isRustSourceKind(f.kind));
          if (!target) {
            return { isPanelOpen: true, lessonVisible: false, panelTitle: title, phase: { status: "idle" } };
          }
          return {
            isPanelOpen: true,
            lessonVisible: false,
            ...
            project: {
              ...
              files: s.project.files.map((f) => (f.id === target.id ? { ...f, content: source, dirty: false } : f)),
            },
          };
        }),
