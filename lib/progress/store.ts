  ...
  persist(
    (set, get) => ({
      ...
      activityLog: [
        {
          ...
          subtitle: "Welcome to REEC Academy. Ready to begin Phase 00.",
          timestamp: Date.now(),
          iconType: "code",
        },
      ],
          ...
          activityLog: [
            {
              ...
              subtitle: "All lessons, times, and streak history cleared.",
              timestamp: Date.now(),
              iconType: "code",
            },
          ],
      ...
      getStreak: () => {
        ...
        const best = Math.max(current, current > 0 ? current : 0);

        // Generate past 7 days (Monday through Sunday or last 7 calendar days)
        ...
        return { current, best, daysStatus, todayActive };
      },
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        // Gracefully sanitize legacy entries without crashing
        const sanitizedActivities: ActivityItem[] = Array.isArray(p.activityLog)
          ? p.activityLog
              .filter((act) => act && !act.path?.startsWith("/labs"))
              .map((act) => ({
                ...act,
