...
export default function ProjectsPage() {
  ...
  if (projects.length === 0) {
    return (
      <div className="h-full overflow-y-auto py-8 px-4 sm:px-6 lg:px-12 max-w-4xl mx-auto flex flex-col justify-center items-center text-center space-y-6">
  ...
  return (
    <div className="h-full overflow-y-auto py-6 sm:py-8 px-4 sm:px-6 lg:px-12 max-w-[1600px] mx-auto space-y-8 scroll-smooth">
      {/* Header Banner */}
      <div
        className="relative overflow-hidden rounded-[26px] border border-slate-200/60 dark:border-white/[0.08] bg-white/75 dark:bg-[#0c1322]/80 p-6 sm:p-10 backdrop-blur-2xl backdrop-saturate-150 shadow-xs"
        style={{
          boxShadow:
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          ...
          <Button
            onClick={() => setModalOpen(true)}
            className="h-10 px-5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shrink-0 self-start md:self-center shadow-md flex items-center gap-2"
          >
            <Plus size={14} />
            <span>Create Project</span>
          </Button>
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
          <button
            ...
            className={cn(
              "whitespace-nowrap shrink-0 rounded-2xl px-3.5 py-2 text-xs font-semibold transition-all duration-150 border backdrop-blur-md shadow-xs",
              selectedPhase === "all"
                ? "border-blue-500/40 bg-blue-600 text-white shadow-xs"
                : "border-slate-200/70 dark:border-white/[0.08] bg-white/70 dark:bg-white/[0.04] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200"
            )}
          >
        ...
        <div className="relative w-full md:w-72 shrink-0">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-slate-200/70 dark:border-white/[0.08] bg-white/70 dark:bg-white/[0.04] pl-9 pr-3 py-2 text-xs text-slate-800 dark:text-slate-200 placeholder:text-slate-400 outline-none backdrop-blur-md focus:border-blue-500/50 shadow-xs transition-all"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-6 items-start pb-28 sm:pb-32 lg:pb-8">
            ...
            {filteredProjects.map((proj) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "group relative cursor-pointer overflow-hidden rounded-[22px] border p-4 sm:p-5 transition-all duration-200 backdrop-blur-xl shadow-xs",
                    isSelected
                      ? "border-blue-500/50 bg-blue-500/10 dark:bg-blue-600/15 shadow-md"
                      : "border-slate-200/60 dark:border-white/[0.06] bg-white/70 dark:bg-[#0c1424]/70 hover:border-blue-500/30 hover:bg-white/90 dark:hover:bg-[#0c1424]/90"
                  )}
                  style={{
                    boxShadow: isSelected
                      ? "inset 0 1px 0 rgba(255, 255, 255, 0.4), 0 8px 20px -4px rgba(59, 130, 246, 0.15)"
        ...
        {currentProj && (
          <div className="md:col-span-7 space-y-6">
            <div
              className="rounded-[26px] border border-slate-200/70 dark:border-white/[0.08] bg-white/80 dark:bg-[#0c1424]/85 p-6 backdrop-blur-2xl backdrop-saturate-150 shadow-xs space-y-5"
              style={{
                ...
                <div className="space-y-2">
                  {currentProj.milestones.map((milestone, idx) => (
                    <div
                      ...
                      className={cn(
                        "flex items-start gap-3 rounded-2xl border p-3.5 cursor-pointer transition-all backdrop-blur-md",
                        milestone.completed
                          ? "border-emerald-500/30 bg-emerald-500/5 text-slate-800 dark:text-slate-200"
                          : "border-slate-200/60 dark:border-white/[0.06] bg-slate-50/50 dark:bg-white/[0.02] hover:border-blue-500/30"
                      )}
                    >
                      {milestone.completed ? (
