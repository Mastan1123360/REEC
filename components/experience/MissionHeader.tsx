  ...
  return (
    ...
    >
      <div className="mb-3.5 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
        ...
        <span className="text-amber-500 font-mono tracking-widest text-[11px]">
          {DIFFICULTY_LABEL[model.difficulty]}
        </span>
      </div>

      <div className="mb-2.5 inline-flex items-center gap-1.5 rounded-full border border-blue-500/30 bg-blue-500/10 px-3 py-1 text-xs font-semibold text-blue-600 dark:text-blue-400">
        <Compass size={13} />
      ...
      {model.learningObjectives.length > 0 && (
        ...
        >
          <div className="mb-2.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
            <Target size={14} className="text-blue-500" /> Learning Objectives
          </div>
          <ul className="space-y-1.5 text-xs sm:text-sm text-slate-700 dark:text-slate-300">
            {model.learningObjectives.map((obj, i) => (
              <li key={i} className="flex gap-2 items-start">
                <ListChecks size={14} className="mt-0.5 shrink-0 text-blue-600 dark:text-blue-400" />
                <span>{obj}</span>
              </li>
            ))}
          </ul>
      ...
      {model.keyTerms.length > 0 && (
        <div className="mt-4 flex flex-wrap items-center gap-1.5">
          ...
          {model.keyTerms.map((t) => (
            <Badge key={t} variant="outline" className="font-mono text-[10.5px]">
              {t}
            </Badge>
          ))}
        </div>
